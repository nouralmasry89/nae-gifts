...
import { getNewbornSub } from "@/lib/newbornSubs";
import { waLink } from "@/lib/categories";

export const Route = createFileRoute("/newborn/$sub")({
  loader: ({ params }) => {
    const sub = getNewbornSub(params.sub);
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.sub.name} — مولود جديد — N.A.E` },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 py-20 text-center">
        ...
        <Link to="/category/$slug" params={{ slug: "newborn" }} className="mt-4 inline-block text-primary hover:underline">
          العودة لمولود جديد
        </Link>
      </div>
      <Footer />
  ...
  component: NewbornSubPage,
});

function NewbornSubPage() {
  const { sub } = Route.useLoaderData();
  const orderMsg = `مرحباً، أود الاستفسار عن: ${sub.name} (مولود جديد)`;
      ...
      <main>
        <section className="relative">
          ...
          <div className="mx-auto -mt-24 max-w-6xl px-4">
            <Link to="/category/$slug" params={{ slug: "newborn" }} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary">
              <ArrowLeft className="h-4 w-4 rotate-180" /> مولود جديد
            </Link>
