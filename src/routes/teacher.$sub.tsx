...
import { getTeacherSub } from "@/lib/teacherSubs";
import { waLink } from "@/lib/categories";

export const Route = createFileRoute("/teacher/$sub")({
  loader: ({ params }) => {
    const sub = getTeacherSub(params.sub);
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.sub.name} — هدايا عيد المعلم — N.A.E` },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 py-20 text-center">
        ...
        <Link to="/category/$slug" params={{ slug: "teacher" }} className="mt-4 inline-block text-primary hover:underline">
          العودة لهدايا عيد المعلم
        </Link>
      </div>
      <Footer />
    </div>
  ...
  component: TeacherSubPage,
});

function TeacherSubPage() {
  const { sub } = Route.useLoaderData();
  const orderMsg = `مرحباً، أود الاستفسار عن: ${sub.name} (هدايا عيد المعلم)`;
      ...
      <main>
        <section className="relative">
          ...
          <div className="mx-auto -mt-24 max-w-6xl px-4">
            <Link to="/category/$slug" params={{ slug: "teacher" }} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary">
              <ArrowLeft className="h-4 w-4 rotate-180" /> هدايا عيد المعلم
            </Link>
