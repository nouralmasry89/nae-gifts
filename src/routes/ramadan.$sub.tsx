...
import { getRamadanSub } from "@/lib/ramadanSubs";
import { waLink } from "@/lib/categories";

export const Route = createFileRoute("/ramadan/$sub")({
  loader: ({ params }) => {
    const sub = getRamadanSub(params.sub);
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.sub.name} — رمضان والأعياد — N.A.E` },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen">
      ...
      <div className="mx-auto max-w-6xl px-4 py-20 text-center">
        ...
        <Link to="/category/$slug" params={{ slug: "ramadan" }} className="mt-4 inline-block text-primary hover:underline">
          العودة لرمضان والأعياد
        </Link>
      </div>
      <Footer />
    </div>
  ...
  component: RamadanSubPage,
});

function RamadanSubPage() {
  const { sub } = Route.useLoaderData();
  const orderMsg = `مرحباً، أود الاستفسار عن: ${sub.name} (رمضان والأعياد)`;
      ...
      <main>
        <section className="relative">
          ...
          <div className="mx-auto -mt-24 max-w-6xl px-4">
            <Link to="/category/$slug" params={{ slug: "ramadan" }} className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary">
              <ArrowLeft className="h-4 w-4 rotate-180" /> رمضان والأعياد
            </Link>
